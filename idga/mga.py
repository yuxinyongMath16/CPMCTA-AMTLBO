...
def track_map(optimal_chromosome, uavs, targets, airports, Obstacles, return_airport):
    ...
    filename = "task_assignment_" + ".png"
    plt.savefig(filename)


def generate_initial_population(scenario, init_solution):
    # 动态生成函数名（新增部分）
    func_name = f"fixed_population_new{scenario}_{init_solution}"

    # 安全检查：防止非法函数名
    if not hasattr(population_generation_fix, func_name):
        available = [name for name in dir(population_generation_fix)
                     if name.startswith(f"fixed_population_new{scenario}_")]
        raise ValueError(
            f"初始化方案 {init_solution} 不可用 (场景 {scenario})\n"
            f"可用方案: {[s.split('_')[-1] for s in available]}"
        )

    # 动态获取函数（新增部分）
    population_func = getattr(population_generation_fix, func_name)
    return population_func()


def main(targets, uavs, Obstacles, population, max_iter, Ne, Ncr, Nm, threat_r, scenario, init_solution):
    start = time.time()
    move_air_time = []
    ...
    iteration_f2_value = []
    # 程序初始化
    # chromosomes, resources = population_generation_fix.fixed_population_new11_1()
    chromosomes, resources = generate_initial_population(scenario, init_solution)
    # rand_num1,rand_num2 = Roulette(pop_fitness,target,UAV)
    # print(rand_num1,rand_num2)
    iter_num = 0
    while True:

        pop_fitness, time_sum, return_airports, pop_f1, pop_f2 = unlock.fitness(chromosomes, targets, uavs, Obstacles, threat_r)
        iter_num = iter_num + 1
        ...
        new_chromosomes = []
        new_resources = []
        pop_fitness_sort = sorted(range(len(pop_fitness)), key=lambda k: pop_fitness[k])
        for i in range(Ne):
            new_chromosomes.append(copy.deepcopy(chromosomes[pop_fitness_sort[i]]))
            new_resources.append(copy.deepcopy(resources[pop_fitness_sort[i]]))
        for i in range(Ncr):
            c_chromosome1, c_chromosome2, c_resource1, c_resource2 = crossover.crossover(chromosomes, resources, pop_fitness)
            # print('迭代',iter_num,'次后的染色体///111///交叉的变化-----后：',c_chromosome1)
            # print('迭代',iter_num,'次后的染色体///222///交叉的变化-----后：',c_chromosome2)
            # print('交叉后的染色体：',c_chromosome1,c_chromosome2)
        ...
        # print('种群交叉次数：@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@',iter_num)
        for j in range(Nm):
            m_chromosome, m_resource = mutation.mutation(chromosomes, resources)
    ...
    optimal_chromosome = chromosomes[optimal_index]
    optimal_chromosome_airport = return_airports[optimal_index]
    # print('最优方案为：', optimal_chromosome)
    # print('最优方案的机场', optimal_chromosome_airport)
    # print('最优方案f1', iteration_f1_value[-1])
    # print('最优方案f2', iteration_f2_value[-1])
    end = time.time()
    run_time = end - start
    # print("Runtime is ：", run_time)  # 计时结果！！！
    # print('程序计算移动机场总耗时是', sum(move_air_time))
    return optimal_chromosome, iteration_optimal_value, run_time, iteration_f1_value, iteration_f2_value
...
if __name__ == '__main__':
    ...
    iter_num = 100
    threat_r = 2500
    optimal_chromosome, return_airport, _, _, _ = main(targets, uavs, Obstacles, population, iter_num, Ne, Ncr, Nm, threat_r)
