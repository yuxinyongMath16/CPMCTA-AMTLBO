    ...
    for i in range(len(uavs)):
        if uavs[i].type == 3:
            ...
            if sum_task == 0:
                ...
                pst = math.exp(0.11/((sum_task-number_of_tasks[i])/sum_task-0.89))
                Pst.append(pst)
...
def coding_population1(targets, uavs, population_size):
    solutions = []  # 一个种群的解
    number = 0  # 当前已生成染色体的数量
    resources = []  # 一个种群所有解的资源约束集
    while number < population_size:
...
def exchanging_two_columns(solution_fit, targets, uavs, Obstacles):
    ...
    fitness, _, _ = unlock.fitness([solution_fit[0]], targets, uavs, Obstacles)
    solution_fit[1] = fitness[0]
    # print('适应度计算后', solution_fit)
    return solution_fit


def sorting_solution(solutions, targets, uavs, Obstacles):
    fitness, _, _ = unlock.fitness(solutions, targets, uavs, Obstacles)
    solution_fit_set = []
    for i in range(len(fitness)):
        solution_fit_set.append([solutions[i], fitness[i]])
...
if __name__ == '__main__':
    ...
    #           [[15000, 23000], [1, 2, 3], 70, 2], [[18000, 60000], [1, 2, 3], 70, 2], [[-10000, 45000], [1, 2, 3], 70, 2]]
    # 序号16：8U20T算例数据
    airport = [[[42000, -2000], 16, math.pi], [[38000, 30000], 60, 3 * math.pi / 4], [[54000, 15000], 16, math.pi / 2],
               [[-16000, 10000], 50, math.pi / 2]]
    Uav = [[[42000, -2000], 1, 80, 0, 250], [[42000, -2000], 3, 60, 5, 220], [[38000, 30000], 3, 70, 5, 210],
    ...
    for i in range(len(Uav)):
        ...
        uavs.append(c_uav)
    population = 50
    # chromosomes, resources = coding_population(targets, uavs, Obstacles, population)
    for j in range(3):
        chromosomes, resources = coding_population1(targets, uavs, population)
        print('查看染色体：', chromosomes)
        print('查看资源约束集', resources)
